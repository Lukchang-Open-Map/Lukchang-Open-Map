-- Insert sample security zone using the Lukchang focus area coordinates
-- This creates a polygon security zone that matches the existing focus area

INSERT INTO public.security_zones (name, description, geometry, is_active)
VALUES (
  'Lukchang Campus Security Zone',
  'Main security zone covering the Lukchang campus area',
  ST_GeomFromGeoJSON('{
    "type": "Polygon",
    "coordinates": [[
...
-- Verify the insert
SELECT id, name, description, is_active, ST_AsGeoJSON(geometry) as geometry_json
FROM public.security_zones
WHERE name = 'Lukchang Campus Security Zone';
